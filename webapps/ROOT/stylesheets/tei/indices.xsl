    ...
    <xsl:if test="doc[str[@name='index_item_name']][not(str[@name='index_thesaurus_hierarchy'])]">
      ...
      <div id="intro-text">
        <xsl:choose>
          ...
          <p>The places listed below are mere geographic references, which have been employed in order to locate on the map the second-level records and the keywords. All the types of transactions or legal actions linked to them are referred to the keywords enumerated at the beginning of each entry.
            <br/>One will find two different lists in this section:
            <br/>1) the list containing all the places which correspond to an individual item (expressed as places/<i>number</i>); this item has been marked up in at least one document;
            <br/>2) the list of all the places whose names have not been standardised, and which do not correspond to an individual item.
            <br/>Please note that it may be necessary to zoom in on the map in order to see more clearly the place that one intends to visualise.
            <br/>Please click on ‘Show all Linked Items’ in order to display all the links between the items of this section and the second-level records that have been created.
            <br/>Please note that by clicking on the ‘See on Map’ button you will display all the places linked to the item you have selected.</p>
        </xsl:when>
          <xsl:when test="doc[str[@name='index_item_number' and starts-with(., 'juridical_persons')]]">
            <p>This section includes religious and ecclesiastical institutions, as well as political collective bodies (urban communes in particular).
              <br/>One will find two different lists in this section:
              <br/>1) the list containing all the juridical persons which correspond to an individual item (expressed as juridical_persons/<i>number</i>); this item has been marked up in at least one document;
              <br/>2) the list of all the juridical persons whose names have not been standardised, and which do not correspond to an individual item.
              <br/>Please click on ‘Show all Linked Items’ in order to display all the links between the items of this section and the second-level records that have been created.
              <br/>Please note that by clicking on the ‘See on Map’ button you will display all the places linked to the item you have selected.</p>
          </xsl:when>
          <xsl:when test="doc[str[@name='index_item_number' and starts-with(., 'estates')]]">
            <p>This section lists the landed estates and the jurisdictional ambits within which fiscal lands and/or the exertion of public rights are recorded.
              <br/>One will find two different lists in this section:
              <br/>1) the list containing all the estates which correspond to an individual item (expressed as estates/<i>number</i>); this item has been marked up in at least one document;
              <br/>2) the list of all the estates whose names have not been standardised, and which do not correspond to an individual item.
              <br/>Please click on ‘Show all Linked Items’ in order to display all the links between the items of this section and the second-level records that have been created.
              <br/>Please note that by clicking on the ‘See on Map’ button you will display all the places linked to the item you have selected.</p>
          </xsl:when>
          <xsl:when test="doc[str[@name='index_item_number' and starts-with(., 'people')]]">
            <p>People’s names have been standardised according to the critical edition of the so-called <i>Liber vitae</i> from the abbey of S. Salvatore/S. Giulia di Brescia (Brescia, Biblioteca Civica Queriniana, G. VI. 7), which was redacted after the middle of the ninth century and updated until the fourteenth century with a high number of names (including lists of oblates and obituaries).
              <br/>The years following each person’s name can indicate birth-death dates, period(s) in office, or attestation(s).
              <br/>One will find three different lists in this section:
              <br/>1) the list containing all the names which have been standardised, and which correspond to an individual item (expressed as people/<i>number</i>); this item has been marked up in at least one document;
              <br/>2) the list of all the names that have been standardised, but do not correspond to an individual item;
              <br/>3) the list of the names that have not been standardised, and do not correspond to an individual item.
              <br/>Please click on ‘Show all Linked Items’ in order to display all the links between the items of this section and the second-level records that have been created.
              <br/>Please note that by clicking on the ‘See on Map’ button you will display all the places linked to the item you have selected.</p>
          </xsl:when>
      </xsl:choose>
