...
<xsl:stylesheet exclude-result-prefixes="#all"
  ...
  <xsl:import href="epidoc-index-utils.xsl" /> <!-- where there are some variables and templates that are used below -->
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="all_mentions">
      <xsl:for-each select="$root//tei:div[@type='edition']//tei:geogName/@ref"><xsl:value-of select="concat(' ', replace(normalize-space(.), '#', ''), ' ')"/></xsl:for-each>
    </xsl:variable>
    <xsl:variable name="not_mentioned" select="$estates/tei:place/tei:geogName[not(@type='other')][not(@type='coord')][not(.='XXX')][not(contains($all_mentions, concat(' ', normalize-space(following-sibling::tei:idno), ' ')))]"/> 
    <xsl:variable name="id-values">
      <xsl:for-each select="//tei:geogName[ancestor::tei:div/@type='edition'][@ref!='']/@ref|$not_mentioned/following-sibling::tei:idno">
    ...
    <add>
      <xsl:for-each select="$ids">
        ...
        <xsl:variable name="element-id" select="$estates/tei:place[translate(translate(child::tei:idno, '#', ''), ' ', '')=$el-id][child::tei:geogName!=''][1]"/>
        <xsl:variable name="item" select="$root//tei:geogName[ancestor::tei:div/@type='edition'][@ref!=''][contains(concat(' ', translate(@ref, '#', ''), ' '), concat(' ', $el-id, ' '))]|$not_mentioned"/>
        <doc>
          ...
          <field name="index_item_name">
            <xsl:choose>
              <xsl:when test="$element-id">
                <xsl:apply-templates mode="italics" select="$element-id/tei:geogName[1]" />
              </xsl:when>
              <xsl:when test="$el-id and not($element-id)"><xsl:value-of select="$el-id" /></xsl:when>
            </xsl:choose>
          </field>
          <xsl:if test="$element-id/tei:geogName[@type='other']//text()">
            <field name="index_other_names">
              <xsl:apply-templates mode="italics" select="$element-id/tei:geogName[@type='other']"/>
          </field>
          </xsl:if>
          ...
          <xsl:if test="$element-id/tei:idno='estates/100'"><!-- to prevent having this indexed for all instances -->
            <field name="index_total_items">
            <xsl:value-of select="string(count($estates/tei:place[not(child::tei:geogName='XXX')]))"/>
          </field>
          </xsl:if>
          
          <xsl:variable name="all_keys">
            <xsl:for-each select="$root//tei:geogName[translate(replace(@ref, ' #', '; '), '#', '')=$el-id][@key]">
          ...
          <xsl:variable name="i_linked_places"> <!-- places linked to linking/linked estates -->
            <xsl:for-each select="$linking_estates|$links[@type='estates']/@corresp[.!='']">
          ...
          <xsl:variable name="i_linking_places"><!-- places linking to linking/linked estates -->
            <xsl:for-each select="$places/tei:place/tei:link[@type='estates'][@subtype='correspondsTo']/@corresp[.!='']">
              <xsl:for-each select="distinct-values(tokenize(., '\s+'))">
                <xsl:variable name="link" select="translate(., '#', '')"/>
                <xsl:if test="contains(concat($linking_estates, ' '), concat($link, ' ')) or contains(concat($linked_estates, ' '), concat($link, ' '))">
                  <xsl:value-of select="$places/tei:place[contains(concat(string-join(child::tei:link[@type='estates'][@subtype='correspondsTo']/@corresp, ' '), ' '), concat($link, ' '))]/tei:idno"/><xsl:text> </xsl:text></xsl:if>
                  ...
                  <xsl:choose>
                    ...
                    <xsl:when test="$all_items/tei:*[child::tei:idno=$key][1]/tei:link[contains(concat(translate(@corresp, '#', ''), ' '), concat($idno, ' '))][@subtype!='']">
                      ...
                      <xsl:choose>
                        ...
                        <xsl:when test="$reverse='correspondsTo'"><xsl:text>correspondsTo</xsl:text></xsl:when>
  ...
  <xsl:template match="tei:geogName[ancestor::tei:div[@type='edition']]">
